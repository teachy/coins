...
public class HuobiTest5 {
    ...
    private void task() throws Exception {
        ...
        if (!volume.equals("0")) {
            if (buyOrSell == 1) {
                if (cost_open - last_price > 50 && checkList(last_price)) {
                    open("buy", buySize + "");
                }
            }
            if (buyOrSell == -1) {
                if (last_price - cost_open > 50 && checkList(last_price)) {
                    open("sell", buySize + "");
                }
            }
            if (volume.substring(0, volume.indexOf(".")).equals(buySize + "")) {
                if (buyOrSell == 1) {
