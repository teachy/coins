...
public class TzTask {
    ...
    private int bsJS = 1;
    private int bsFK = 1;
    private int beeting = 5000;

    //    @Scheduled(cron = "0/5 * * * * ?")
    public void tzForFK() throws IOException, InterruptedException {
        if (!yesterday.equals(getYesterday())) {
            ...
            logger.info("tzForFK:" + yesterday + ":" + jgListforFK);
        }
        if (jgListforFK.size() > 4) {
            DD dd = get28(kfget);
            logger.info("fk" + dd.toString());
            Thread.sleep(((int) (Math.random() * Integer.parseInt(dd.getJcTime())) - 5) * 1000);
            ...
            tz28(dd, beeting * bsFK, postFK);
            Thread.sleep(30000);
        }
    }

    @Scheduled(cron = "0/30 * * * * ?")
    public void tzForJS() throws IOException, InterruptedException {
        if (!yesterday.equals(getYesterday())) {
            yesterday = getYesterday();
            jgListforJS.clear();
            ...
            logger.info("tzForJS:" + yesterday + ":" + jgListforJS);
        }
    ...
    private void tz28(DD dd, int betting, HttpPost post) throws IOException {
        int sum = 0;
        StringBuffer bet_num = new StringBuffer();
        List<BasicNameValuePair> formParams1 = new ArrayList<BasicNameValuePair>();
        for (int ii = 0; ii < jgListforJS.size(); ii++) {
            sum += bet[((Integer) jgListforJS.get(ii)).intValue()];
        }
        int[] betting1 = new int[28];
        int fenshu = betting / sum;
        for (int i = 0; i <= 27; i++) {
            betting1[i] = (bet[i] * fenshu);
            if (jgListforJS.contains(Integer.valueOf(i))) {
